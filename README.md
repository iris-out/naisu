...
- **하드클린 / 클린 / 원본 / 둘 다** — 저장 방식을 고릅니다. 하드클린은 이미지를 JPEG(.jpg)로 완전히 재인코딩하는 가장 강력한 제거 방식이고(컨테이너·알파채널·EXIF가 통째로 사라짐, 대신 투명도 소실 + 손실압축), 클린은 프롬프트 등 생성 정보만 제거(.webp 유지), 원본은 그대로 저장, 둘 다는 클린 파일과 원본 파일을 함께 저장합니다.
