...
### 자동 다운로드 (상세 기능)
저장 방식은 네 가지 중 골라 씁니다: **하드클린**(픽셀을 다시 그려 재인코딩, 가장 확실하지만 화질 살짝 저하), **클린**(EXIF, 알파 채널 은닉만 무손실 제거, WebP 유지), **원본**(그대로 저장), **둘 다**(클린 + 원본 `_raw`를 함께 저장). 이번 한 장만 다른 방식으로 저장할 수도 있습니다. 다운로드 화면에서 품질(100/90/80/70/60)과 출력 포맷(NAI 기본인 WebP 또는 JPEG)을 고를 수 있습니다. 파일명은 `{datetime}`, `{seed}`, `{model}`, `{w}`, `{h}`, `{steps}`, `{idx}` 같은 토큰으로 구성하고, 배치 없이 화면의 이미지 한 장만 즉시 저장할 수도 있습니다.
